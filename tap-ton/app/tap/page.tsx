...
export default function TapPage() {
  ...
  useEffect(() => {
    // Safely get Telegram user on the client side after component mounts
    ...
    const unsubscribe = onAuthStateChanged(auth, async (user) => {
      if (user) {
        ...
        if (userSnap.exists()) {
          ...
          const tgUser = getTelegramUser(); // get user again inside the auth scope
          if (tgUser) {
            await setDoc(userRef, {
              tgId: tgUser.tgId, // Use tgId from the safe getter
